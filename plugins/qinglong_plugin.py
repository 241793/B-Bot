...
async def cs(message, middleware):
    if not await middleware.is_admin(message["user_id"]):
        return {"content": "您没有权限执行此操作。", "to_user_id": message["user_id"]}
    # content = message.get('content', '').strip()
    # parts = content.split()
    ...
    await asyncio.get_running_loop().run_in_executor(None, lambda: client.add_envs(d))
    return {
        "content": f"添加cs666'...",
        "to_user_id": message["user_id"]
    }
