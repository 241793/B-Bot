...
- [适配器对接](#适配器对接)
  - [常见适配器快速对接](#常见适配器快速对接)
    - [QQ（llonebot）](#qqllonebot--正向-ws)
    - [官方 QQ Bot](#官方-qq-bot)
    - [wxclaw](#wxclaw微信-clawbot)
...
|---|---|
| 控制台 | 总览、对话调试、适配器启停 |
| 本地管理 | 插件、数据桶、规则、Python 依赖、本地图床 |
| 任务与脚本 | 计划任务、外部容器、**内置青龙**（环境变量 / 定时 / 脚本 / 订阅 / 依赖 / 日志） |
| AI 能力 | AI 大脑、子智能体 Agent、MCP、技能、知识库、工作流、AI 定时、记忆、Bot-Chat |
| 支付 | 订单、支付宝 / 码支付配置 |
| 系统 | 系统配置、日志、在线文档、安全审计 |
...
下面是最常用渠道的最短配置说明。更复杂的 Custom 对接见下一节。
...
支持以下环境变量（本地 / Docker 均可）：

| 变量 | 说明 | 默认 |
|---|---|---|
| `qq_HOST` | 反向 WebSocket 监听主机 | `0.0.0.0` |
| `qq_PORT` | 反向 WebSocket 端口（如 QQ 等） | `8888`（路径常为 `/ws/qq`） |
| `WEB_UI_HOST` | Web 管理界面主机 | `0.0.0.0` |
| `WEB_UI_PORT` | Web 管理界面端口 | `5000` |

更多项可在 Web「系统配置」中设置（Docker 代理、GitHub 代理、日志清理等）。

---

## 插件开发

阅读顺序：开发规范 → Python 插件 → Middleware 实践 / API 速查 → JS 插件 → Webhook。
...
> 入口：Web 管理后台左侧菜单「AI 大脑」。
>
> 阅读顺序：功能总览 → 配置中心 → 技能中心 → 知识中心 → 工作流中心 → AI 定时任务 → 调用链路 → 排错 → 知识库导出。
...
1. **插件热加载**：支持动态启用/禁用、在线编辑和实时保存（保存后会尝试重载）。
2. **规则优先级**：数值越大优先级越高。
...
| 问题 | 排查建议 |
|---|---|
| 在线文档不更新 | 检查系统配置「GitHub 代理」是否缓存；清空代理后重开文档，或看页脚来源/hash |
| 插件调试无输出 | 确认左侧编辑器代码已应用；检查 `__pattern__`/`register`；连续对话需在超时内回复 |
